<aside :class="sidebarCollapsed ? 'w-20' : 'w-72'"
    ...
    <nav class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
        ...
        @if(auth()->user()->role === 'manager')
            ...
            <a href="{{ route('customers.index') }}"
               class="group relative flex items-center gap-3 rounded-xl px-3 py-3 {{ request()->routeIs('customers.index') ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition-all duration-200">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('profile.edit') }}"
               class="flex items-center gap-3 rounded-xl p-2 hover:bg-slate-800 transition-colors flex-1 overflow-hidden">
                <div
                    class="h-10 w-10 shrink-0 rounded-full bg-slate-700 flex items-center justify-center text-white font-bold overflow-hidden border border-slate-600 group-hover:border-emerald-500 transition-colors">
                ...
                <div x-show="!sidebarCollapsed" class="flex-1 overflow-hidden transition-opacity duration-200">
                    ...
                    <p class="truncate text-xs text-slate-500 capitalize">{{ auth()->user()->role ?? 'User' }}</p>
                </div>
            </a>
